...
const TripPlanNo1 = () => {
    ...
    const db1 = firestore.collection("역사관광").doc("대구광역시").collection("달서구");
    db1.get().then((result)=>{
        result.forEach((allDoc)=>{
            user.push(allDoc.data())
        })
    ...
    const new_script = src => { 
      ...
      useEffect(()=>{
        ...
        // 마커 하나를 지도위에 표시합니다 
        addMarker(new kakao.maps.LatLng(35.851743, 128.5598102));
        where.map((element)=>(
...
async function deleteClickLine() {
    if (clickLine) {
...
async function showDistance(content, position) {
...
async function displayCircleDot(position, distance) {
...
async function deleteCircleDot() {
    var i;
...
async function getTimeHTML(distance) {
